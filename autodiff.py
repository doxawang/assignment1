...
class Node(object):
    ...
    def matmul(self,other,trans_A=False,trans_B=False):
        new_node = matmul_op(self,other,trans_A,trans_B)
        return new_node
...
class SubOp(Op):
    ...
    def gradient(self, node, output_grad):
        """Given gradient of add node, return gradient contributions to each input."""
        return [output_grad, mul_byconst_op(output_grad, -1)]
...
class SquareSumOp(Op):
    def __call__(self, node_A):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A]
        new_node.name = "SquareLoss(%s)" % node_A.name
        return new_node
    
    def compute(self, node, input_vals):
        assert(len(input_vals)==1)
        return np.sum(input_vals[0]**2) / 2.0
    
    def gradient(self, node, output_grad):
        return [mul_op(output_grad, node.inputs[0])]
